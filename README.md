...
- Los endpoints abiertos (`/api/purchases`, `/api/warranties` y la preparación de una activación) tienen un límite de
  pedidos por minuto y por dirección (`src/lib/server/rate-limit.ts`).
- El fee de una transacción firmada por el usuario se compara con el de su simulación antes de que la cuenta emisora
  la pague.
