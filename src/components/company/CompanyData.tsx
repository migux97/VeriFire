...
const formatDate = (value: string | null) =>
  value ? new Intl.DateTimeFormat('es-AR', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(value)) : 'Pendiente';
...
export function CompanyData({ view = 'dashboard' }: { view?: 'dashboard' | 'products' | 'activity' }) {
  ...
  if (failed)
    return (
      <p className="company-data-empty" role="alert">
        No se pudieron cargar todas las compras. Recargá la página para consultar los datos completos.
      </p>
    );
    ...
    <>
      <section className="company-panel company-products company-real-products">
        <div className="company-panel-heading">
          <div>
            <span className="company-eyebrow">Inventario real</span>
            <h2>{view === 'activity' ? 'Estado de los productos' : 'Productos de tu cuenta'}</h2>
          </div>
          <a href="#batches">
            Ver lotes <i className="fa-solid fa-arrow-right" />
          </a>
        </div>
        {loading ? (
          <p className="company-data-empty">Cargando los datos de tu cuenta...</p>
        ) : products.length === 0 ? (
          <p className="company-data-empty">Todavía no hay productos asociados a esta cuenta. Genera tu primer lote para verlos aquí.</p>
        ) : (
                ...
                {products.map((product) => (
                  <tr key={product.token}>
                    <td>
                      ...
                      <small>Lote {product.lot}</small>
                    </td>
                    <td>{product.model}</td>
                    <td>
                      <span className={`status-pill ${product.status === 'CLAIMED_IN_WARRANTY' ? 'status-warning' : 'status-ok'}`}>
                        <i className="fa-solid fa-circle" />
                        {product.status === 'CLAIMED_IN_WARRANTY' ? 'Activado' : 'Sellado'}
                      </span>
                    </td>
