...
export function CompanyOverview({ records, loading }: { records: PurchaseStatus[]; loading: boolean }) {
  ...
  if (!records.length)
    return (
      <section className="company-real-products">
        <h2>Tu actividad, en un solo lugar</h2>
        <p className="company-data-empty">Todavía no hay compras disponibles. Cuando generes un lote, aquí verás sus indicadores y porcentajes.</p>
      </section>
    );
  ...
  const metrics = [
    ...
    { label: 'Productos activados', value: number(claimed), detail: `${number(activation)}% de las unidades`, icon: 'circle-check', color: 'green' },
    {
      ...
      detail: `${number(percentage(pending, records.length))}% de las compras`,
      icon: 'clock',
      color: 'amber'
    },
  ...
  return (
    <div className="company-overview">
      ...
      <div className="company-overview-grid">
        <section className="company-real-products">
          <span className="company-eyebrow">Últimos seis meses</span>
          <h2>Unidades por mes de compra</h2>
          <p className="company-chart-caption">Compras que ya tienen un lote generado.</p>
          <div className="company-bars" role="img" aria-label={months.map((month) => `${month.label}: ${month.count} unidades`).join('; ')}>
            {months.map((month) => (
              <div className="company-bar-column" key={month.label}>
                <strong>{number(month.count)}</strong>
                <div className="company-bar-track">
                  <div className="company-bar" style={{ height: `${(month.count / max) * 100}%` }} />
                </div>
        ...
        <section className="company-real-products">
          <span className="company-eyebrow">Garantías</span>
          <h2>Porcentaje de activación</h2>
          <div
            className="company-donut"
            style={{ '--activation': `${activation}%` } as CSSProperties}
            role="img"
            aria-label={`${number(activation)}% de unidades activadas`}
          >
            <div>
              <strong>{number(activation)}%</strong>
              <span>activado</span>
            </div>
          </div>
          <dl className="company-chart-legend">
            ...
            <div>
              <dt>Sin activar</dt>
              <dd>{number(Math.max(0, units - claimed))}</dd>
            </div>
          </dl>
        </section>
        <section className="company-real-products company-overview-status">
          <span className="company-eyebrow">Estado de las compras</span>
          <h2>Distribución operativa</h2>
          {[
            { label: 'Con lote generado', count: issued.length },
            { label: 'Pendientes de generación', count: pending }
          ].map((item) => (
            <div className="company-progress-row" key={item.label}>
              ...
              <progress max={records.length} value={item.count} aria-label={item.label} />
            </div>
          ))}
        </section>
      </div>
      <section className="company-real-products">
        <span className="company-eyebrow">Detalle informativo</span>
        <h2>Compras y activaciones por lote</h2>
          ...
          <table className="company-table">
            ...
            <thead>
              <tr>
                ...
                <th scope="col">Activadas</th>
                <th scope="col">Activación</th>
                <th scope="col">Importe</th>
              </tr>
            </thead>
            <tbody>
              {records.map(({ purchase, succeeded }) => (
                <tr key={purchase.purchaseId}>
                  ...
                  <td>{number(purchase.quantity)}</td>
                  <td>{number(purchase.claimed)}</td>
