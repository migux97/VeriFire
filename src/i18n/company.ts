...
const es = {
  ...
  settings: {
    ...
    verification: {
      ...
      lead: 'Verifire confirma quién es tu empresa. Hasta entonces, lo que ve un comprador al escanear tus productos dice que están "registrados", no que son "originales", y no pueden aparecer en la página de inicio.',
      status: { none: 'Sin verificar', pending: 'En revisión', verified: 'Verificada', rejected: 'Rechazada', paused: 'Pausada' },
      verifiedText: (name: string) => `Verifire confirmó que ${name} es una empresa real. Tus compradores ven "Empresa verificada por Verifire" en cada producto, y tus productos pueden aparecer en la página de inicio.`,
      domain: (domain: string) => `Sitio confirmado: ${domain}`,
      pausedText: (name: string) => `Tu empresa fue verificada como «${name}», pero hoy mostrás otro nombre a tus compradores. Volvé a publicar la marca con ese nombre, o pedí una nueva verificación.`,
      pendingText: (date: string) => `Recibimos tu solicitud el ${date}. Cuando la revisemos, el estado cambia acá.`,
      rejectedText: 'No pudimos verificar tu empresa.',
      reason: 'Motivo',
      checklist: 'Para revisarla usamos los datos de tu perfil de empresa',
      needs: { legalName: 'Razón social', taxId: 'CUIT o identificación fiscal', website: 'Sitio web', email: 'Correo de contacto' },
      optional: 'opcional',
      needProfile: 'Completá la razón social y el CUIT en el perfil para poder pedir la verificación.',
      message: 'Algo más para la revisión (opcional)',
      messagePlaceholder: 'Ej. Sitio oficial, perfil de la empresa en un registro público, persona de contacto.',
      request: 'Solicitar verificación',
      requestAgain: 'Volver a solicitarla',
      working: 'Enviando...',
      requested: 'Solicitud enviada. Cuando la revisemos, el estado cambia acá.',
      failed: 'No se pudo enviar la solicitud:',
      readOnly: 'Solo el administrador de la empresa puede pedir la verificación.'
    },
    brand: {
...
const en: CompanyMessages = {
  ...
  settings: {
    ...
    verification: {
      ...
      lead: 'Verifire confirms who your company is. Until then, what a buyer sees when scanning your products says they are "registered", not that they are "original", and they cannot appear on the home page.',
      status: { none: 'Not verified', pending: 'Under review', verified: 'Verified', rejected: 'Rejected', paused: 'Paused' },
      verifiedText: (name: string) => `Verifire confirmed that ${name} is a real company. Your buyers see "Company verified by Verifire" on every product, and your products can appear on the home page.`,
      domain: (domain: string) => `Confirmed site: ${domain}`,
      pausedText: (name: string) => `Your company was verified as “${name}”, but you now show your buyers another name. Publish the brand again with that name, or ask for a new verification.`,
      pendingText: (date: string) => `We received your request on ${date}. When we review it, the status changes here.`,
      rejectedText: 'We could not verify your company.',
      reason: 'Reason',
      checklist: 'To review it we use the data of your company profile',
      needs: { legalName: 'Legal name', taxId: 'Tax ID', website: 'Website', email: 'Contact email' },
      optional: 'optional',
      needProfile: 'Fill in the legal name and the tax ID in the profile to ask for the verification.',
      message: 'Anything else for the review (optional)',
      messagePlaceholder: 'E.g. Official site, the company’s page in a public registry, a contact person.',
      request: 'Request verification',
      requestAgain: 'Request it again',
      working: 'Sending...',
      requested: 'Request sent. When we review it, the status changes here.',
      failed: 'The request could not be sent:',
      readOnly: 'Only the company administrator can ask for the verification.'
    },
    brand: {
